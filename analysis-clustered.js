...
const analysis = authors => {
    ...
    const triplets = (nodes, links) => {
		...
		// K-Means clustering based on nodes xy
		let nodeXY = []
        for (let i1 = 0; i1 < nodes.length; i1++) {
			// prepare node xy data for K-Means clustering 
            const n1 = nodes[i1]			
			nodeXY.push([n1.x,n1.y])
        }
		var res = skmeans(nodeXY, clusters);
		console.log('Clustered authors #', res)
		let millefeuille1 = []
		let millefeuille2 = []
        for (let i1 = 0; i1 < nodes.length; i1++) {
            const n1 = nodes[i1]
			n1.clusterid = res.idxs[i1]
			millefeuille1.push([n1.clusterid, n1.nationality, 1])
			for(var key in n1.nationalities) {
			  var counts = n1.nationalities[key];
			  millefeuille2.push([n1.clusterid, n1.nationality, key, counts])
			}
		}
    ...
    const writing = (nodes, links, triplets, millefeuille1, millefeuille2) => {
        ...
        fs.writeFile('./src/data/nodes-clustered.json', JSON.stringify(nodes), err => { if (err) throw err })
        fs.writeFile('./data/nodes-clustered.json', JSON.stringify(nodes, null, '\t'), err => { if (err) throw err })
